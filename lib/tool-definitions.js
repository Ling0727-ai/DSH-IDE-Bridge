...
function renderSymbols(args, value) {
  if (Array.isArray(value?.symbols)) {
    const symbols = args?.include_low_value
      ? value.symbols
      : value.symbols.filter(symbol => !isLowValueRenderedSymbol(symbol, value.symbols))
...
function isLowValueRenderedSymbol(symbol, symbols) {
  if (isLowValueRenderedKind(symbol.kind, symbol.namePath)) return true
  const kind = String(symbol.kind ?? '').toLowerCase()
  if (!kind.includes('variable')) return false
  const start = (symbol.location ?? symbol).range?.start
  if (!start) return false
  return symbols.some(container => {
    if (container === symbol || !isFunctionLikeKind(container.kind)) return false
    const range = (container.location ?? container).range
    return rangeContains(range, start)
  })
}

function isLowValueRenderedKind(kind, namePath) {
  const value = String(kind ?? '').toLowerCase()
  if (['import', 'binding', 'specifier', 'parameter', 'property', 'field', 'reference', 'definitionexpression', 'literal', 'functionexpression'].some(fragment => value.includes(fragment))) return true
  if (value.includes('variable') && String(namePath ?? '').includes('/')) return true
  return false
}

function isFunctionLikeKind(kind) {
  const value = String(kind ?? '').toLowerCase()
  return value.includes('function') || value.includes('method') || value.includes('constructor') || value.includes('lambda')
}
...
function comparePosition(left, right) {
  ...
  return (left.column ?? left.character ?? 0) - (right.column ?? right.character ?? 0)
}
